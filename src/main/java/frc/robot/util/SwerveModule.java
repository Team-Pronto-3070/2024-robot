...
public class SwerveModule {
  ...
  public SwerveModule(int driveMotorID, int turnMotorID, double encoderOffset) {
    ...
    currentLimitsConfig.SupplyCurrentLimitEnable =
      Constants.Swerve.Drive.enableCurrentLimit;
    currentLimitsConfig.SupplyCurrentLimit =
    ...
    OpenLoopRampsConfigs.DutyCycleOpenLoopRampPeriod =
      Constants.Swerve.Drive.openLoopRamp;
    ...
    ClosedLoopRampsConfigs.DutyCycleClosedLoopRampPeriod =
      Constants.Swerve.Drive.closedLoopRamp;
    ...
    turningAbsoluteEncoder.setPositionConversionFactor(
      Constants.Swerve.Turn.encoderPositionFactor
    );
    turningAbsoluteEncoder.setVelocityConversionFactor(
      Constants.Swerve.Turn.encoderVelocityFactor
    );
    ...
    turningPID.setPositionPIDWrappingMaxInput(
      Constants.Swerve.Turn.encoderPositionFactor
    );
    ...
    turningPID.setOutputRange(
      Constants.Swerve.Turn.PID.minOutput,
      Constants.Swerve.Turn.PID.maxOutput
    );
  ...
  public void setDesiredState(
    SwerveModuleState2 rawDesiredState,
    boolean isOpenLoop
  ) {
    ...
    if (isOpenLoop) {
      driveMotor.set(
        // ControlModeValue.DutyCycleOut,
        optimizedDesiredState.speedMetersPerSecond / Constants.Swerve.maxSpeed
      );
    } else {
      // double wheelRPM =
      // (
      //    (optimizedDesiredState.speedMetersPerSecond * 60) /
  ...
  public SwerveModuleState2 getState() {
    return new SwerveModuleState2(
      (
        (
          // driveMotor.getSelectedSensorVelocity() * // raw falcon units TODO: put correct values here
          (600.0 / 2048.0) / // motor RPM
          Constants.Swerve.gearRatio
        ) * // wheel RPM
        Constants.Swerve.wheelCircumference
      ) / // wheel surface speed in meters per minute
